...
const { ps, getChildrenOfPid } = require("ps-get");
...
class XClap extends EventEmitter {
  ...
  async _getProcs() {
    if (!this._procs) {
      this._procs = await ps();
    }
    return this._procs;
  }

  _terminateChildren(children) {
  ...
  async cleanProcesses() {
    // terminate all other processes
    const procs = await this._getProcs();
    const children = getChildrenOfPid(process.pid, procs);
    this._terminateChildren(children);
  }
  ...
  killTaskChildren() {
    setTimeout(async () => {
      ...
      for (const sym of symbols) {
        const ch = children[sym];
        const cc = await getChildrenOfPid(ch.pid, procs);
        ch.kill();
        this._terminateChildren(cc);
      }
    }, 10);
  }
